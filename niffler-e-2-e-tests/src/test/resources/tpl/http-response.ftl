...
<#if data.body??>
    <h4>Body</h4>
    <div>
        <#-- Проверяем, является ли тело HTML -->
        <#if data.body?contains("<html") || data.body?contains("<div") || data.body?contains("<p")>
            <pre><code class="html">${data.body?html}</code></pre>
        <#elseif data.body?starts_with("{") || data.body?starts_with("[")>
            <pre><code class="json">${data.body?html}</code></pre>
        <#else>
